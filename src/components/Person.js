...
function Person({person}) {
  ...
  // Key help react identofy which item ina list have changed or added or removed and plays a crucil role in handling UI updates efficiently 
  return ( 
   <h2>
...
// With key the react match the children in the original tree with the children in the subsequent tree ,Now the react knows that the element with the id = 3 is the new one and the the elements with the key 1 & 2 have just moved(so the react will preserve the subtree and simply insert the new item at the top of the list)